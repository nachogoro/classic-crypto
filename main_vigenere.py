...
class VigenereApp(tk.Tk):
    ...
    def update_cipher_key(self):
        ...
        if not new_key or len(set(new_key).difference(set(self.alphabet()))) > 0:
            # Invalid symbols or empty key
            self.cipher_key_text.set("")
            self.cipher_key = [0]
            return

        self.cipher_key = [self.alphabet().index(c.upper()) for c in new_key]


    def translate(self, message: str, mode: Mode) -> str:
        result = []
        src_alphabet = self.alphabet()

        for index in range(len(message)):
            offset = self.cipher_key[index % len(self.cipher_key)]
            dst_alphabet = shift_list(src_alphabet, offset if mode == Mode.ENCRYPTION else -offset)

            capitalized = message[index].upper()
            ...
            result += (translated.upper() if message[index].isupper() else translated.lower())
    ...
    def __init__(self):
        ...
        guide_label = tk.Label(key_frame, text="Vigènere key:", font=self.default_font)
