...
class VigenereApp(tk.Tk):
    ...
    def uppercase_alphabet(self):
        return [c.upper() for c in alphabet_utils.lowercase_alphabet(self.selected_language)]
    ...
    def update_cipher_key(self):
        ...
        if not new_key or len(set(new_key).difference(set(self.uppercase_alphabet()))) > 0:
            # Invalid symbols or empty key
            self.cipher_key_text.set("")
            self.cipher_key = [0]
            return

        self.cipher_key = [self.uppercase_alphabet().index(c.upper()) for c in new_key]
    ...
    def translate(self, message: str, mode: Mode) -> str:
        ...
        for index in range(len(message)):
            ...
            if not alphabet_utils.is_in_alphabet(self.selected_language, to_translate):
                result += to_translate
                continue

            offset = self.cipher_key[processed_letters % len(self.cipher_key)]
            if mode == Mode.DECRYPTION:
                offset = -offset

            result += alphabet_utils.shifted_letter(to_translate,
